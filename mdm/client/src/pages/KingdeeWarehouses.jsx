...
export default function KingdeeWarehouses() {
  ...
  useEffect(() => {
    api.get('/kingdee/organizations', { params: { entityType: 'warehouse' } })
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await api.get('/kingdee/data', {
        params: { entityType: 'warehouse', page: page + 1, pageSize: rowsPerPage, keyword: debouncedKeyword, orgName },
      });
  ...
  return (
    <Box>
      <Typography variant="h5" fontWeight="bold" mb={2}>金蝶仓库数据</Typography>
      <Typography variant="body2" color="text.secondary" mb={1}>共 {total} 个仓库（含所有组织）</Typography>
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          <TableHead><TableRow>
            <TableCell>编码</TableCell><TableCell>名称</TableCell><TableCell>所属组织</TableCell>
            <TableCell>所属分组</TableCell><TableCell>地址</TableCell><TableCell>仓库属性</TableCell>
            <TableCell>联系电话</TableCell><TableCell>最后同步</TableCell>
          </TableRow></TableHead>
          <TableBody>
            {records.map(r => {
              const extra = r.extra || {};
              return (<TableRow key={r.id} hover>
                ...
                <TableCell>{extra.groupName || '-'}</TableCell>
                <TableCell sx={{ maxWidth: 180 }}><Typography variant="body2" noWrap title={extra.address || ''}>{extra.address || '-'}</Typography></TableCell>
                <TableCell>{extra.stockProperty || '-'}</TableCell>
                <TableCell>{extra.tel || '-'}</TableCell>
                <TableCell>{r.lastSyncAt ? new Date(r.lastSyncAt).toLocaleString('zh-CN') : '-'}</TableCell>
              </TableRow>);
            })}
            {records.length === 0 && !loading && (
              <TableRow><TableCell colSpan={8} align="center">暂无仓库数据，请先从金蝶拉取</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </TableContainer>
