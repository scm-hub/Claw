...
export default function KingdeeSuppliers() {
  ...
  useEffect(() => {
    api.get('/kingdee/organizations', { params: { entityType: 'supplier' } })
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await api.get('/kingdee/data', {
        params: { entityType: 'supplier', page: page + 1, pageSize: rowsPerPage, keyword: debouncedKeyword, orgName },
      });
  ...
  return (
    <Box>
      <Typography variant="h5" fontWeight="bold" mb={2}>金蝶供应商数据</Typography>
      <Typography variant="body2" color="text.secondary" mb={1}>共 {total} 条供应商记录</Typography>
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          <TableHead><TableRow>
            <TableCell>编码</TableCell><TableCell>名称</TableCell><TableCell>简称</TableCell>
          ...
          <TableBody>
            ...
            {records.length === 0 && !loading && (
              <TableRow><TableCell colSpan={5} align="center">暂无供应商数据，请先从金蝶拉取</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </TableContainer>
