...
const STATUS_LABELS = { PENDING: '待收款', PARTIAL: '部分收款', SETTLED: '已结清', OVERDUE: '逾期' };
...
export default function ReceivableList() {
  ...
  useEffect(() => { api.get('/master/customers?page=1&pageSize=999').then(r => setCustomers(r.data?.list || [])); }, []);

  return (
    <Box>
      <Typography variant="h5" sx={{ mb: 2, fontWeight: 700 }}>应收账款</Typography>

      {/* 汇总卡片 */}
      <Grid container spacing={2} sx={{ mb: 2 }}>
        <Grid item xs={3}><Card><CardContent><Typography color="textSecondary" variant="body2">应收总额</Typography><Typography variant="h5" fontWeight={700}>¥{Number(summary.amount || 0).toLocaleString()}</Typography></CardContent></Card></Grid>
        <Grid item xs={3}><Card><CardContent><Typography color="textSecondary" variant="body2">已收金额</Typography><Typography variant="h5" color="success.main" fontWeight={700}>¥{Number(summary.receivedAmount || 0).toLocaleString()}</Typography></CardContent></Card></Grid>
        <Grid item xs={3}><Card><CardContent><Typography color="textSecondary" variant="body2">未收余额</Typography><Typography variant="h5" color="error.main" fontWeight={700}>¥{Number(summary.balance || 0).toLocaleString()}</Typography></CardContent></Card></Grid>
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell>编号</TableCell><TableCell>客户</TableCell><TableCell>来源</TableCell>
              <TableCell>应收金额</TableCell><TableCell>已收</TableCell><TableCell>余额</TableCell>
          ...
          <TableBody>
            {list.map((r) => (
              <TableRow key={r.id} hover>
                <TableCell>{r.arNo}</TableCell>
                <TableCell>{r.customer?.name}</TableCell>
                <TableCell>{r.refType === 'SALES_ORDER' ? '销售订单' : r.refType}</TableCell>
                <TableCell>¥{Number(r.amount).toLocaleString()}</TableCell>
                <TableCell>¥{Number(r.receivedAmount).toLocaleString()}</TableCell>
                ...
                <TableCell><IconButton size="small" onClick={async () => { const res = await api.get(`/finance/receivable/${r.id}`); setDetail(res.data); }}><Visibility /></IconButton></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        ...
        <DialogContent>
          {detail && (
            <Grid container spacing={2} sx={{ mt: 0.5 }}>
              <Grid item xs={6}><Typography>编号: {detail.arNo}</Typography></Grid>
              <Grid item xs={6}><Typography>客户: {detail.customer?.name}</Typography></Grid>
              ...
              <Grid item xs={6}><Typography>应收: ¥{Number(detail.amount).toLocaleString()}</Typography></Grid>
              <Grid item xs={6}><Typography>已收: ¥{Number(detail.receivedAmount).toLocaleString()}</Typography></Grid>
              ...
              {detail.invoice && <Grid item xs={12}><Typography>关联发票: {detail.invoice.invoiceNo}</Typography></Grid>}
            </Grid>
          )}
        </DialogContent>
