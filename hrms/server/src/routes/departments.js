...
// 导出部门列表（Excel）
router.get('/export', authenticate, authorize('SUPER_ADMIN', 'HR_ADMIN'), async (req, res, next) => {
  try {
    // 递归查询所有部门及其员工数
    ...
    ws['!cols'] = [
      { wch: 20 }, { wch: 18 }, { wch: 12 }, { wch: 14 }, { wch: 14 }, { wch: 10 }, { wch: 14 },
    ];
    ...
    const empDepts = await prisma.department.findMany({
      include: {
        ...
        employees: {
          select: {
            employeeNo: true, name: true, gender: true, phone: true,
            position: true, status: true, hireDate: true,
          },
    ...
    empDepts.forEach((dept) => {
      if (dept.employees.length === 0) {
        detailRows.push({
          '部门': dept.name, '上级部门': dept.parent?.name || '无',
          '工号': '', '姓名': '', '性别': '', '手机号': '',
          '岗位': '', '状态': '', '入职日期': '',
        });
      } else {
        dept.employees.forEach((emp) => {
          detailRows.push({
            '部门': dept.name, '上级部门': dept.parent?.name || '无',
            '工号': emp.employeeNo, '姓名': emp.name,
            '性别': genderMap[emp.gender] || emp.gender,
            '手机号': emp.phone, '岗位': emp.position,
    ...
    ws2['!cols'] = [
      { wch: 20 }, { wch: 18 }, { wch: 10 }, { wch: 10 }, { wch: 6 },
      { wch: 14 }, { wch: 12 }, { wch: 8 }, { wch: 14 },
    ];
...
// 导入部门（Excel）
router.post('/import', authenticate, authorize('SUPER_ADMIN', 'HR_ADMIN'), upload.single('file'), async (req, res, next) => {
  try {
    if (!req.file) {
    ...
    // 查找可用列名（兼容中英文）
    const nameKey = Object.keys(rows[0]).find(k => k.includes('部门名称') || k.toLowerCase().includes('name')) || Object.keys(rows[0])[0];
    const parentKey = Object.keys(rows[0]).find(k => k.includes('上级部门') || k.toLowerCase().includes('parent')) || null;
    const managerKey = Object.keys(rows[0]).find(k => k.includes('负责人') && !k.includes('工号') && !k.includes('岗位') || k.toLowerCase() === 'manager') || null;
    ...
    // 加载已有部门
    const existingDepts = await prisma.department.findMany({ select: { id: true, name: true, parentId: true } });
    ...
    for (let i = 0; i < sortedRows.length; i++) {
      ...
      if (parentKey) {
        ...
        if (parentName && parentName !== '无' && parentName !== '-' && parentName !== '0') {
          ...
          if (!parentId) {
            results.errors.push(`第${i + 2}行「${name}」：上级部门「${parentName}」不存在，已跳过`);
            results.skipped++;
            continue;
          }
        }
      ...
      if (managerKey) {
        ...
        if (managerName && managerName !== '无' && managerName !== '-') {
          ...
          if (emp) {
            ...
            results.errors.push(`第${i + 2}行「${name}」：负责人「${managerName}」未找到在职员工匹配，已跳过负责人`);
          }
        }
      }
...
// 批量调整部门排序
router.put('/reorder', authenticate, authorize('SUPER_ADMIN', 'HR_ADMIN'), async (req, res, next) => {
  try {
...
router.post('/init-sort', authenticate, authorize('SUPER_ADMIN', 'HR_ADMIN'), async (req, res, next) => {
  try {
    // 获取所有部门按 parentId 分组
    const allDepts = await prisma.department.findMany({
...
// 获取导入模板
router.get('/import-template', authenticate, authorize('SUPER_ADMIN', 'HR_ADMIN'), async (req, res, next) => {
  try {
    const templateData = [
  ...
  try {
    ...
    const dept = await prisma.department.create({
      data: {
        ...
        sortOrder: req.body.sortOrder !== undefined ? req.body.sortOrder : (await prisma.department.count({ where: { parentId: parentId || null } })),
      },
      include: {
        children: true,
...
router.put('/:id', authenticate, authorize('SUPER_ADMIN', 'HR_ADMIN'), async (req, res, next) => {
  try {
    ...
    if (name && name.trim() !== existing.name) {
      const dup = await prisma.department.findFirst({ where: { name: name.trim(), id: { not: deptId } } });
      if (dup) {
        return res.status(400).json({ success: false, message: '该部门名称已存在' });
      }
    }
...
// 删除部门
router.delete('/:id', authenticate, authorize('SUPER_ADMIN', 'HR_ADMIN'), async (req, res, next) => {
  try {
    const deptId = req.params.id;
    ...
    if (dept.children.length > 0) {
      return res.status(400).json({ success: false, message: '该部门下有子部门，请先删除或迁移子部门' });
    }

    // 检查是否有员工
    if (dept.employees.length > 0) {
      return res.status(400).json({ success: false, message: '该部门下有员工，请先迁移员工到其他部门' });
    }
