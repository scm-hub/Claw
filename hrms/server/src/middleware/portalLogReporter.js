/**
 * 操作日志上报中间件 — HRMS 子系统 → Portal 统一日志中心
 *
...
const FIELD_LABELS = {
  ...
  level: '级别', priority: '优先级', sortOrder: '排序',
  departmentId: '部门', positionId: '职位ID',
};

const MODEL_CONFIG = {
  'employees':   { model: 'employee',    labelFields: ['name'], diffFields: ['name', 'gender', 'phone', 'email', 'status', 'departmentId', 'positionId'] },
  'departments': { model: 'department',  labelFields: ['code', 'name'], diffFields: ['code', 'name', 'description'] },
  'positions':   { model: 'position',    labelFields: ['name', 'code'], diffFields: ['code', 'name', 'level', 'description'] },
  'users':       { model: 'user',        labelFields: ['email'], diffFields: ['email', 'role', 'name'] },
  'roles':       { model: 'role',        labelFields: ['name'], diffFields: ['name', 'description', 'level'] },
};
